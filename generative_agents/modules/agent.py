...
class Agent:
    ...
    def find_path(self, agents):
        ...
        # Filter out empty paths (unreachable destinations)
        valid_pathes = {t: p for t, p in pathes.items() if len(p) > 0}
        if not valid_pathes:
            # All destinations are unreachable, return empty path
            self.logger.warning(f"{self.name}: All target tiles are unreachable from {self.coord}")
            return []
        
        # Find the shortest valid path
        target = min(valid_pathes, key=lambda p: len(valid_pathes[p]))
        return valid_pathes[target][1:]
    ...
    def _determine_action(self):
        ...
        self.logger.debug(f"{self.name}: Calling describe_object for {address[-1]}")
        obj_describe = self.completion("describe_object", address[-1], describes[-1])
        self.logger.debug(f"{self.name}: describe_object returned: {obj_describe}")
        
        if obj_describe is None:
            self.logger.warning(f"{self.name}: describe_object returned None, using failsafe")
            obj_describe = "空闲"
        
        self.logger.debug(f"{self.name}: Creating obj_event with describe: {obj_describe}")
        obj_event = self.make_event(address[-1], obj_describe, address)
        self.logger.debug(f"{self.name}: obj_event created successfully")

        event.emoji = f"{de_plan['describe']}"

        self.logger.debug(f"{self.name}: Creating Action with duration: {de_plan['duration']}, start: {de_plan['start']}")
        ...
        self.logger.debug(f"{self.name}: Action created successfully")
        return action
