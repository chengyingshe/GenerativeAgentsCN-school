    ...
    def prompt_describe_object(self, obj, describe):
        ...
        def _callback(response):
            ...
            try:
                ...
                if result is None:
                    print(f"Warning: parse_llm_output returned None for {obj}. Response: {response[:200]}")
                    return "空闲"
                ...
                # If parsing fails, log the error and return failsafe
                print(f"Warning: Failed to parse describe_object output for {obj}. Error: {e}")
                print(f"Response was: {response[:200]}")
                return "空闲"
            except Exception as e:
                # Catch any other exceptions
                print(f"Warning: Unexpected error in describe_object callback for {obj}: {type(e).__name__}: {e}")
                print(f"Response was: {response[:200]}")
                return "空闲"
